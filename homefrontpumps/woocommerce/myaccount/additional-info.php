...
<form id="regForm" method="POST" action="<?php echo esc_url(admin_url('admin-post.php')) ?>" enctype="multipart/form-data" novalidate>
    ...
    <div class="form-group" id="option_set1" >
		...
		<label class="additional-info-label">Have you received a breast pump for this birth?  <span class="ast">*</span></label>		
        <select name="receptionConfirmation" class="form-control" id="receptionConfirmation" required>
                <option value="" disabled selected>select</option>
				<?php $saved_receptionConfirmation = isset( $receptionConfirmation ) && ( $receptionConfirmation == 'Yes' ) ? 'selected' : ''; ?>
                <option value="Yes"  <?php echo $saved_receptionConfirmation; ?> >Yes</option>
                <option value="No"  <?php echo $saved_assigned_sex; ?> >No</option>
        </select>
