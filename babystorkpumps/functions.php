...
require_once __DIR__ . '/payers/category_additional_fields.php';
...
function wp_register_scripts_styles()
{
	...
	if (!is_admin()) {
		...
		wp_register_script('singup_form',  get_template_directory_uri() . "/templates/onestep-form/signup_form.js", array('jquery', 'intlTelInput'), time());
		wp_register_script('dmez-payer',  get_template_directory_uri() . "/templates/onestep-form/dmez-payer.js", array('jquery'));

		/**
		 * Styles
		 */
		wp_register_style('poppinscss', 'https://fonts.googleapis.com/css2?family=Poppins&display=swap');
		wp_register_style('dosiscss', 'https://fonts.googleapis.com/css2?family=Dosis:wght@600;700&display=swap');
...
{
	wp_enqueue_style('poppinscss');
	wp_enqueue_style('dosiscss');
...
function add_site_field_to_order( $order_id ){
	// action...
	if ( ! add_post_meta( $order_id, 'site', 'babystorkpumps.com', true ) ) { 
		update_post_meta ( $order_id, 'site', 'babystorkpumps.com' );
	 }
}
...
function user_add_sitemetafield( $user_id ) {
 	update_user_meta($user_id, 'site', 'babystorkpumps.com');
}


add_action('updated_user_meta', function($meta_id = null, $object_id = null, $meta_key = null, $meta_value = null){
	
	if ($meta_key !== 'user_modified_flag') {
		update_user_meta($object_id, 'user_modified_flag', true);
		error_log('BSP user '. $object_id . ' changed ' . $meta_key);
	}	
});
...
function sender_email( $original_email_address ) {
	return 'support@babystorkpumps.com';
}
add_filter( 'wp_mail_from_name', 'sender_name' );
function sender_name( $original_email_from ) {
	return 'Baby Stork Pumps';
}