...
function multi_step_form_submission()
{
	...
	// Commented this as now we allow bags with 'Yes' #KHFP-96 on Jira
	//    if ( $post_data['receptionConfirmation'] == 'yes' ) {
	...
	order_free_product($products, $post_data['firstname'], $post_data['lastname'], $post_data['streetAddress'] . $post_data['unit'], $post_data['city'], $post_data['state'], $post_data['postcode'], $user_id, $post_data['email']);
...
function order_free_product($products, $shipping_first_name, $shipping_last_name, $shipping_address_1, $shipping_city, $shipping_state, $shipping_postcode, $user_id, $user_email, $form_name="multi_step_form")
{
...
function homefrontpump_register_user($first_name, $last_name, $email, $password)
{
	...
	error_log('before inserting new user');
	$user_id  = wp_insert_user($userdata);
	...
	error_log('affiliate_id is ' . $affiliate_id);
	$is_active_affiliate = affwp_is_active_affiliate($affiliate_id);
	error_log('is_active_affiliate ');
	if ($is_active_affiliate) error_log('true');
...
function homefrontpump_register_login()
{
	...
	// Indicate step 1 submission
	$multi_step_form_progress_meta = get_user_meta($user_id, 'multi_step_form_progress', true);
	if (empty($multi_step_form_progress_meta)) {
		update_user_meta($user_id, 'multi_step_form_progress', 1);
	}
...
function homefrontpump_save_step_2()
{
	...
	if (!empty($post_data['doctorNpiNumber'])) {
	...
	// Indicate step 2 submission
	$multi_step_form_progress_meta = get_user_meta($user_id, 'multi_step_form_progress', true);
	if ($multi_step_form_progress_meta < 2) {
		update_user_meta($user_id, 'multi_step_form_progress', 2);
	}
	update_user_meta($user_id, 'user_modified_flag', true); // For API checks
}
...
{ ?>
	...
	<table class="form-table">
		...
		<tr>
			<th><label for="register_page">Registered Page</label></th>
			<td>
		...
		<tr>
			<th><label for="doctorNpiNumber">Doctor/OB NPI Number</label></th>
			<td>
				<input type="text" name="doctorNpiNumber" id="doctorNpiNumber" value="<?php echo esc_attr(get_user_meta($user->ID, 'doctorNpiNumber', true)); ?>" class="regular-text" /><br />
			</td>
		</tr>

		<tr>
			<th><label for="primaryInsurance">Name of Medical Insurance</label></th>
			<td>
				<input type="text" name="primaryInsurance" id="primaryInsurance" value="<?php echo esc_attr(get_user_meta($user->ID, 'primaryInsurance', true)); ?>" class="regular-text" required /><br />
			</td>
		</tr>
		<tr>
			<th><label for="memberId">Member ID Number</label></th>
			<td>
				<input type="text" name="memberId" id="memberId" value="<?php echo esc_attr(get_user_meta($user->ID, 'memberId', true)); ?>" class="regular-text" required /><br />
			</td>
		</tr>
		<tr>
			<th><label for="groupNumber">Group Number (if applicable)</label></th>
			<td>
				<input type="text" name="groupNumber" id="groupNumber" value="<?php echo esc_attr(get_user_meta($user->ID, 'groupNumber', true)); ?>" class="regular-text" required /><br />
			</td>
		</tr>
		<tr>
			<th><label for="subscriberName">Name of Subscriber</label></th>
			<td>
				<input type="text" name="subscriberName" id="subscriberName" value="<?php echo esc_attr(get_user_meta($user->ID, 'subscriberName', true)); ?>" class="regular-text" required /><br />
			</td>
		</tr>
		<tr>
...
function validateProfileFields(WP_Error &$errors)
{
	...
	if (empty($_POST['doctorNpiNumber'])) {
		$errors->add('doctorNpiNumber', "<strong>ERROR</strong>: Doctor Npi Number is required.");
	}
	if (empty($_POST['primaryInsurance'])) {
		$errors->add('primaryInsurance', "<strong>ERROR</strong>: Medical Insurance is required.");
	}
	if (empty($_POST['memberId'])) {
		$errors->add('memberId', "<strong>ERROR</strong>: Member Id is required.");
	}
	if (empty($_POST['subscriberName'])) {
		$errors->add('subscriberName', "<strong>ERROR</strong>:  Name of Subscriber is required.");
	}
...
function save_extra_profile_fields($user_id)
{
	...
	if (!empty($_POST['doctorNpiNumber'])) {
		...
		update_user_meta($user_id, 'doctorNpiNumber', $_POST['doctorNpiNumber']);
	}
	if (!empty($_POST['primaryInsurance'])) {
	...
	if (!empty($_POST['memberId'])) {
		...
		update_user_meta($user_id, 'memberId', $_POST['memberId']);
	}
	if (!empty($_POST['groupNumber'])) {

		update_user_meta($user_id, 'groupNumber', $_POST['groupNumber']);
	}
	if (!empty($_POST['subscriberName'])) {

		update_user_meta($user_id, 'subscriberName', $_POST['subscriberName']);
	}
...
function reorder_action()
{
	...
	$headers = "From: $from";
	$message = "Dear $user_firstname , \n We have received your order and are working on it! If you have any questions, feel free to contact us at support@babystorkpumps.com.";
	wp_mail($user->user_email, 'Baby Stork Pumps', $message, $headers, "-f " . $from);
...
///Added for myaccount/additional-info page///////////////////
